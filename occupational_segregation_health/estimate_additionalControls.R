...
rm(list = ls(all = TRUE))
sink("logs/estimate_additionalControls.txt")

print("Produce alternative specification with additional controls")
...
additionalControls <- counterfactual_estimator(data = d_alt)

# Save the result
saveRDS(additionalControls, file = "intermediate/additionalControls.Rds")
