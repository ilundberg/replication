...
rm(list = ls(all = TRUE))
sink("logs/estimate_afterRedesign.txt")

print("Produce alternative specification after questionnaire redesign")
...
afterRedesign <- counterfactual_estimator(data = d_alt,
...
# Save the result
saveRDS(afterRedesign, file = "intermediate/afterRedesign.Rds")
